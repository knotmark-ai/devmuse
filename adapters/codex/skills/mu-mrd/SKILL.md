...
1. Load references/devmuse/knowledge/principles/premise-check.md
2. Detect context:
...
2. Then produce 5 market sections (one at a time, user approves each):
   ...
   3. **Revenue & opportunity** — who pays, for what, pricing basis, opportunity size (coarse TAM/SAM is enough), and the major cost drivers — opportunity level, not a financial model. When the revenue story needs deeper structure (canvas-level mapping, unit economics, naming), walk the relevant checklist in references/devmuse/knowledge/principles/business-canvases.md and fold the conclusions back into this section.
...
Drafted per references/devmuse/knowledge/principles/prose-discipline.md — conclusion first, derivations shown, negative claims scoped.
...
- **Grill, don't interrogate** — apply references/devmuse/knowledge/principles/grilling.md: one question per message with a recommendation, facts self-served, decisions to the user
...
Before terminal (user-decides in Quick mode, invoke mu-prd in Full mode), consult `references/devmuse/knowledge/principles/sign-off-gate.md`. If stakeholder-scope indicates team-touching, run the gate protocol. Sign-off gate is skipped when stance was `skip`.
...
- **Reads:** references/devmuse/knowledge/principles/premise-check.md (always); references/devmuse/knowledge/principles/stance-detection.md (Phase 0); references/devmuse/knowledge/principles/business-canvases.md (Revenue & opportunity section, when deeper structure is needed); references/devmuse/knowledge/principles/sign-off-gate.md (terminal if team-touching); prior MRD/biz/premise artifacts if present
