...
Read `@../../knowledge/principles/project-context.md` and run `resolve` before
...
7. **C4 positioning** — identify the project's profile(s) per @../../knowledge/principles/project-profiles.md (profiles compose; take the union when unsure), then which C4 levels the approved approach touches per @../../knowledge/principles/architecture-assessment.md. The profile selects which sections and diagrams this design emits; a concern from @../../knowledge/principles/nfr-checklist.md adds a conditional section only when its trigger fires. **Draw only the neighbourhood this change touches**, with the ➕/✏️/➖ overlay; cite `docs/wiki/` pages for the surrounding picture instead of redrawing it. No wiki present → draw what the change needs and record that no architecture map exists.
...
- Before writing Mermaid, read @../../knowledge/principles/mermaid-compat.md and use its compatibility subset; fall back to ASCII when the subset is insufficient
