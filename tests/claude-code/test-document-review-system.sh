...
mkdir -p docs/craft-claude/specs

# Create a spec document WITH INTENTIONAL ERRORS for the reviewer to catch
cat > docs/craft-claude/specs/test-feature-design.md <<'EOF'
# Test Feature Design
...
Read the spec-document-reviewer-prompt.md template in skills/craft-brainstorm/ to understand the review format.

Then review the spec at $TEST_PROJECT/docs/craft-claude/specs/test-feature-design.md using the criteria from that template.
