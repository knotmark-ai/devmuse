...
当仓库选择 GitHub-first 协作且 GitHub 当前可用时，DevMuse 会复用一个匹配的
Issue 承载当前 scope 与交付状态，并在相关 Draft PR 上维护计划修订、进度和审查
证据。每次远端写入仍需重新确认具体操作的能力与用户授权。离线、只读、非
GitHub 或用户拒绝发布时，流程回退到仓库制品并记录原因；受版本控制的项目事实
绝不依赖 checkout 路径或私有缓存。
...
- **mu-model** 🧪 — 领域模型：概念、原型分类、模型主轴、谁产生谁维护。可选的专用工具（不是 PRD 或设计之前的强制门槛），产出仓库根目录的 `CONTEXT.md`。使用 `/mu-model` 调用。**其 `create` 路径尚未在从零开始的项目上验证过**——见 README 的 Validation status。
...
- **mu-setup** — 初始化和维护项目的案例注册表路由与偏好；发现、提议，仅在批准后写入。幂等。使用 `/mu-setup` 调用。
...
| 按需 | **mu-model** 🧪 | 领域模型 — 概念、原型分类、模型主轴、谁产生谁维护；写入 `CONTEXT.md`。可选的专注补充，不是 PRD 或设计之前的门槛 |
| 按需 | **mu-prd** | 产品需求 — 用户流程、对象生命周期模型、线框图、特性规格、分级规则 |
| 按需 | **mu-wiki** | 架构 Wiki — 生成和维护项目级架构文档 |
| 按需 | **mu-grill** | 对方案/设计的穷追式访谈 — 开工前收敛每一个"猜错即返工"的分叉 |
| 按需 | **mu-setup** | 案例注册表初始化 — 发现、提议并（经批准后）写入项目的资产路由并初始化注册表；幂等 |
