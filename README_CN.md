...
**产品层**（每个产品一次；已有项目可选）：
- **mu-biz** — 商业分析：验证前提（quick 模式，4 个质询问题）或完整分析（竞品、BMC、VPC、用户画像、MVP 范围、成本模型）。
- **mu-prd** — 产品需求：用户画像、用户流程、线框图、特性规格、分级规则、NFR、指标。
...
2. **mu-arch** — 在范围确认后激活。专注于技术架构（组件、接口、数据流、错误处理）。提出 2-3 种方案并附架构图，分段展示设计供验证。派遣 mu-reviewer（review-design）审查设计文档。**产品需求用 mu-prd；商业策略用 mu-biz。**
