...
五个技能之间的调用与依赖关系（用户是唯一入口，bootstrap 路由只指路不代调）：

```mermaid
graph TD
    U["用户斜杠直呼<br/>（bootstrap 只指路不代调）"] --> BIZ["/mu-biz"]
    U --> PRD["/mu-prd"]
    U --> WIKI["/mu-wiki"]
    U --> RETRO["/mu-retro"]
    U --> GRILL["/mu-grill"]
...
- **wiki 新鲜度步骤**——数据收集阶段跑一条命令查 `docs/wiki/` 的最后更新日期与之后的提交数；若 `docs/wiki/_index.md` 存在且之后有提交落地，报告落后量（"wiki 落后 N 个提交，最后更新 YYYY-MM-DD"）并**建议** retro 后运行 `/mu-wiki update`。仅建议——mu-retro 从不调用其他技能；无 wiki 时静默跳过。Sources: [skills/mu-retro/SKILL.md:56-59](), [skills/mu-retro/SKILL.md:80](), [skills/mu-retro/SKILL.md:98]()
...
纪律的四条规则，其中"分叉收敛"与"事实 vs 决策"是两大支柱：
...
| 收敛每一个分叉（fork convergence） | **分叉**是猜错就强制返工的决策点，每个分叉必须以用户回答或用户可见的假设收尾。没有问题数预算：在分叉全部解决时停，而不是问题"感觉够了"时停；用户问累了，可提议把剩余分叉转为书面假设——由用户决定，绝不自动封顶 |
