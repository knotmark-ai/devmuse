...
DevMuse 的约束体系分为两个层面：**harness 层的机械强制**（Claude Code 的 PreToolUse hook 在工具调用前拦截，agent 无法用文字绕过）与**文本层的门控**（写在技能体与原则文件里，由 agent 阅读并执行）。前者包括 pipeline gate 与 destructive guard 两个 hook 脚本；后者包括嵌在技能体内、不可协商的 HARD-GATE，以及创作技能出口处可跳过的 sign-off gate。Sources: [hooks/hooks.json:15-36](), [CONTEXT.md:31-41]()

这一分工不是冗余，而是互补：核心管线的顺序由 pipeline gate 在工具层机械强制，同时由 HARD-GATE 在技能体内文本强制。Sources: [CONTEXT.md:81]() 整个体系服从 DevMuse 的“guidance over control”哲学——除 HARD-GATE 外，每条路径都非阻塞，用户可用一个词覆盖任何推荐。Sources: [CONTEXT.md:72-73]()

> **词汇纪律**：本仓库从不使用不加限定的“gate”一词——必须写全称：HARD-GATE / pipeline gate / sign-off gate / size-area gate。四个复合名互斥，无需改名。Sources: [CONTEXT.md:90]()

## Harness 层：hook 注册与触发
...
`session-start` 不属于任何门控家族：它读取 `rules/bootstrap.md`，做 JSON 转义后以 `additionalContext` 注入会话，把 bootstrap 技能装进每个新会话的上下文。它是 harness 层的“供给侧”——让 agent 知道规则存在；两个 PreToolUse hook 才是“约束侧”。Sources: [hooks/session-start:10-33]()
...
2. **插件自编辑豁免**：若目标路径位于 `$CLAUDE_PLUGIN_ROOT/` 之下（即编辑 devmuse 插件自身），立即 `exit 0` 放行——否则维护 DevMuse 自己的技能文件也要先跑一遍 mu-scope/mu-arch，插件将无法自举。Sources: [hooks/pre-tool-use/pipeline-gate.sh:17-25]()
...
两个 PreToolUse 脚本的首行有效语句都是 `trap 'exit 0' ERR`：脚本内任何错误（缺目录、JSON 解析失败、环境异常）都转化为“无决策”退出，Claude Code 照常执行工具调用。Sources: [hooks/pre-tool-use/pipeline-gate.sh:2-5](), [hooks/pre-tool-use/destructive-guard.sh:2-5]()
...
| 信号 | 内容 | 检查 |
|---|---|---|
| S1 | 存在 CODEOWNERS 文件 | `test -f .github/CODEOWNERS \|\| test -f CODEOWNERS` |
| S2 | 近 90 天内 watched-dirs 上有 ≥3 位作者 | `git log --since` 作者去重计数 |
| S3 | 用户明确声明（“team project”、“shared code”、“need RFC”） | 会话内声明 |
...
四个受词汇纪律约束的门控家族（HARD-GATE / pipeline gate / sign-off gate / size-area gate）中，本页覆盖前三个及 harness 层的 destructive guard——size-area gate 属 mu-code 领域，不在此展开：
