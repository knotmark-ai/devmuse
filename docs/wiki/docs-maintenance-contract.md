...
DevMuse 的文档维护契约写在仓库根目录的 `CLAUDE.md` 中，它防范的失效模式是：同一个事实被复制到多个文件后各自静默漂移。这不是假想的风险——在契约存在之前，skill 数量在不同文档里从 10 漂到 11、12、13。契约的核心手段是"一个事实，一个归属"（one fact, one place）：为每类事实指定唯一的权威文件，其他文档只指向它、不复述它。Sources: [CLAUDE.md:1-3](), [CLAUDE.md:5]()
...
| 路由用的技能类别列表 | `rules/bootstrap.md` + `skills/mu-route/SKILL.md` | 这两处必须保持一致 |
...
graph TD
    ...
    F3["事实：路由类别列表"] --> H3["rules/bootstrap.md"]
    F3 --> H4["skills/mu-route/SKILL.md"]
    H3 ---|"两处必须一致"| H4
    F4["事实：域语言 / 术语"] --> H5["CONTEXT.md<br/>（仓库根目录）"]
```

Sources: [CLAUDE.md:5-10](), [CLAUDE.md:23-25]()
...
- **权威表本体**：`README.md` 的 Skills 表列出全部技能的类别与角色（Pipeline / Orthogonal / On-demand / Router / Meta 五类）。Sources: [README.md:92-107]()
...
- **域语言归属**：`CONTEXT.md` 开篇声明它是本仓库的共享词汇表，人类与 agent 在代码、文档、commit、对话中统一使用其术语，`_Avoid_` 下的同义词被刻意弃用；每个词条都附带禁用列表（如 Core pipeline 禁用 "main flow"、"workflow chain"）。Sources: [CONTEXT.md:1-3](), [CONTEXT.md:12-14]()
...
| 4 | `skills/mu-route/SKILL.md` | on-demand 指针行 + on-demand 列表 | 该 skill 为 on-demand 时 |
| 5 | `docs/architecture.md`（+ `docs/architecture_cn.md`） | 派发表 | 该 skill 派遣 agent 时 |

Sources: [CLAUDE.md:13-21]()

"README 中清单出现两次"可在文件中直接验证：叙述形式在 Pipeline 章节（核心管线五步、正交技能、按需技能），表格形式在 Architecture 章节的 Skills 表。Sources: [README.md:37-66](), [README.md:92-107]()
...
这一区分对应 `CONTEXT.md` 中的域语言词条 **Living artifact**：文件名不带日期、原地更新并逐次追加 History 行的文档（explore 产物、wiki、CONTEXT.md 本身），与 `docs/scope|specs|plans` 下的带日期快照相对。wiki 索引模板体现了 living artifact 的机制——记录 Generated 日期、Baseline commit，并以 History 表逐行追加每次生成/更新。Sources: [CONTEXT.md:56-57](), [knowledge/templates/wiki-index.md:1-22]()
