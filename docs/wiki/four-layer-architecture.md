...
理解这套分层的意义在于：它把"始终生效的原则"、"用户触发的流程"、"需要上下文隔离的执行角色"和"按需注入的领域知识"放进了不同的加载通道和 token 成本模型中——始终注入的内容（rules）被刻意压到最小，而可以按需加载的一切都下沉到 skills 与 knowledge。Sources: [docs/architecture.md:77]()

## 四层总览

```
devmuse/
├── rules/        "What must be followed" — 始终生效的原则
...
对 `knowledge/` 还有一组细化标准：只被单个 skill 使用的材料留在该 skill 目录内（局部性优先）；跨场景注入到 agents、语言/框架特定的模式、决策点的思考准则（`principles/`）、特定关注点的评审清单（`reviews/`）才进入 `knowledge/`。Sources: [docs/architecture.md:22-30]()

## 加载机制

四层全部通过插件安装（`claude plugin add`）生效，无需手工配置，但底层机制各不相同：
...
sequenceDiagram
    ...
    CC->>HJ: 会话事件 (startup / clear / compact)
    HJ->>SS: 匹配 SessionStart，执行脚本
    SS->>BS: cat 读取内容
    BS-->>SS: bootstrap 全文
    SS->>SS: JSON 转义 + 包装 EXTREMELY_IMPORTANT
    SS-->>CC: hookSpecificOutput.additionalContext
    CC->>CC: 注入会话上下文，bootstrap 始终在场
```
...
当前仅含一个文件 `bootstrap.md`，承担技能发现与调用规则、指令优先级、决策流程。设计原则是：rules 经 hook 注入、持续消耗 token，因此只放"必须无条件常驻"的内容，可按需加载的一律留在 skills。Sources: [docs/architecture.md:71-77](), [README.md:118-120]()
...
- **领域过滤**：DevMuse 只处理软件工程与产品/商业分析两类工作，域外消息正常回复、不路由。Sources: [rules/bootstrap.md:43-48]()
- **四类技能划分**：Core pipeline（自动路由）、Orthogonal（自动路由）、On-demand（仅 slash 调用）、Meta。未加前缀的域内消息一律先经 `mu-route`，slash 调用则绕过路由。Sources: [rules/bootstrap.md:50-61]()
...
其余 skill 不派遣任何 agent。Sources: [docs/architecture.md:79-90](), [README.md:92-107]()
...
- **rules 只指导、不调用**：bootstrap.md 告诉 Claude 何时调用哪个 skill。
...
```mermaid
graph TD
    R[rules/<br/>bootstrap.md] -->|constrain 约束| S[skills/<br/>mu-xxx 工作流]
