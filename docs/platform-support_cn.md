...
- `mu-review` 的跨模型评审是主机感知且互惠的：Claude Code 交由 Codex 评审，Codex 交由 Claude Code 评审，其它宿主使用配置的不同模型家族评审器，否则无此能力。当前宿主绝不评审自身，深度限制为一，运行时优先使用本地已有的订阅登录而非 API key。评审器 CLI 缺失、认证过期、超时或输出畸形都不会阻塞主评审。
