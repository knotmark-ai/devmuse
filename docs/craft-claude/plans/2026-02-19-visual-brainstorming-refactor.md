...
**Spec:** `docs/craft-claude/specs/2026-02-19-visual-brainstorming-refactor-design.md`
...
| `skills/craft-brainstorm/visual-companion.md` | Modify | Skill instructions: rewrite loop to non-blocking flow |
...
```bash
cd /Users/drewritter/prime-rad/craft-claude && node tests/brainstorm-server/server.test.js
```
...
Search the codebase:
```bash
grep -r "wait-for-feedback" /Users/drewritter/prime-rad/craft-claude/ --include="*.js" --include="*.md" --include="*.sh" --include="*.json"
```
...
**Files:**
- Modify: `skills/craft-brainstorm/visual-companion.md`
...
```bash
git add skills/craft-brainstorm/visual-companion.md
...
```bash
cd /Users/drewritter/prime-rad/craft-claude && lib/brainstorm-server/start-server.sh --project-dir /tmp/brainstorm-smoke-test
```
...
```bash
grep -r "wait-for-feedback\|sendToClaude\|feedback-footer\|send-to-claude\|TaskOutput.*block.*true" /Users/drewritter/prime-rad/craft-claude/ --include="*.js" --include="*.md" --include="*.sh" --include="*.html" | grep -v node_modules | grep -v RELEASE-NOTES | grep -v "\.md:.*spec\|plan"
```
