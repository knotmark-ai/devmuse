...
A skill that is never auto-routed and runs only via explicit slash invocation (mu-mrd, mu-model, mu-prd, mu-wiki, mu-grill, mu-setup); the routing rules answer matching intents with a pointer, not an invocation.
